...
- If no ask-questions tool is available, explicitly say so and end the turn with the smallest necessary list of questions for the user to answer on the next turn.
...
- If the current surface ignores or does not support the visible handoff, explicitly say so and provide a generated context prompt for `psm-project-manager`.
