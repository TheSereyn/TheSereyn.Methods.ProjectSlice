...
test("inspect can read a local installable package source", () => {
    ...
    assert.match(result.stdout, /@thesereyn\/psm v0\.1\.0-alpha\.0/);
...
test("update refreshes a local path package from its source", () => {
    ...
    writeFileSync(sourceToolkit, readFileSync(sourceToolkit, "utf8").replace("version: 0.1.0-alpha.0", "version: 0.1.0-alpha.1"), "utf8");

    const updateResult = runCli(["update", target]);
    assert.equal(updateResult.status, 0, updateResult.stderr || updateResult.stdout);
    assert.match(updateResult.stdout, /Updated package @thesereyn\/psm to v0\.1\.0-alpha\.1/);
    ...
    const lock = JSON.parse(readFileSync(path.join(target, ".psm", "lock.json"), "utf8"));
    assert.equal(lock.packages["@thesereyn/psm"].version, "0.1.0-alpha.1");
});